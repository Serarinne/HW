...
<html lang="en-US" class="scroll-smooth bg-slate-950">
  <head>
    ...
    <title>{{ !empty($query) ? 'Search Characters: ' . $query : 'Honkai: Star Rail Characters Wallpapers' }} - {{ env('APP_NAME') }}</title>
    <meta name="description" content="{{ !empty($query) ? 'Search results for ' . $query . ' Honkai: Star Rail characters on '.env('APP_NAME') : 'Explore thousands of Honkai: Star Rail characters. Discover high-quality HD and 4K wallpapers of your favorite Trailblazers and Stellaron Hunters.' }}" />
    ...
    <link rel="canonical" href="{{ !empty($query) ? route('characters.index', ['q' => $query]) : route('characters.index') }}" />

    <meta property="og:title" content="Honkai: Star Rail Characters Wallpapers | {{ env('APP_NAME') }}" />
    <meta property="og:description" content="Explore thousands of Honkai: Star Rail characters. Discover high-quality HD and 4K wallpapers of your favorite Trailblazers and Stellaron Hunters." />
    ...
    <script type="application/ld+json">
    {
      ...
      "@@type": "CollectionPage",
      "name": "{{ env('APP_NAME') }} HSR Characters Database",
      "description": "Directory of Honkai: Star Rail character wallpaper collections.",
        ...
        <section class="relative z-10 border-b border-white/5 bg-slate-950/50 backdrop-blur-md min-h-[350px] flex flex-col justify-center">
            <div class="w-full max-w-[90rem] mx-auto px-4 sm:px-6 lg:px-8 py-16 sm:py-24 text-center flex flex-col items-center justify-center">
                <h1 class="text-4xl sm:text-5xl lg:text-6xl font-black tracking-tight text-white leading-tight mb-6">
                    ...
                    <span class="text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-cyan-400 drop-shadow-lg">Characters</span>
                </h1>
                <p class="text-base sm:text-lg text-slate-400 font-medium leading-relaxed max-w-2xl mx-auto mb-10">
                    Discover <span class="text-white font-bold">{{ number_format($characters->total()) }}</span> characters from Honkai: Star Rail. Find the perfect HD background of your favorite Astral Express crew or Stellaron Hunters.
                </p>

                <form action="{{ route('characters.index') }}" method="GET" class="w-full max-w-2xl mx-auto group" role="search">
                    <div class="flex items-center rounded-full bg-slate-900/80 backdrop-blur-xl overflow-hidden shadow-xl transition-all duration-300 ring-1 ring-white/10 focus-within:ring-2 focus-within:ring-cyan-500/50 focus-within:shadow-[0_0_30px_rgba(168,85,247,0.15)]">
                        ...
                        <input type="text" name="q" value="{{ $query }}" aria-label="Search character by name" placeholder="Search character by name..." class="w-full bg-transparent px-4 py-4 sm:py-5 text-base sm:text-lg text-white font-medium placeholder:text-slate-400 outline-none" autocomplete="off" />
        ...
        <section class="w-full max-w-[90rem] mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-16 relative z-10">
            <div class="flex flex-col sm:flex-row items-start sm:items-end justify-between mb-8 gap-4 border-b border-white/10 pb-4">
                <div class="space-y-2">
                    ...
                    <h2 class="text-2xl sm:text-3xl font-black text-white">
                        ...
                        @else
                            All <span class="text-slate-400 font-medium">Characters</span>
                        @endif
                    </h2>
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4 sm:gap-6 mb-12">
                @forelse($characters as $character)
                    <!-- Komponen x-character-card akan digunakan di sini -->
