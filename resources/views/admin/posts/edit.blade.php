    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            tinymce.init({
                ...
                images_upload_url: "{{ route('admin.posts.editor-image-upload') }}",
                ...
                images_upload_handler: function (blobInfo, progress) {
                    return new Promise(function (resolve, reject) {
                        ...
                        xhr.open('POST', "{{ route('admin.posts.editor-image-upload') }}");
            ...
            window.createCategory = async function () {
                ...
                try {
                    const response = await fetch("{{ route('admin.posts.ajax-store') }}", {
                        method: 'POST',
                        headers: {
...
<body class="min-h-screen bg-slate-950 text-slate-200 font-sans">
    ...
    <main class="flex-grow pt-8 pb-32 sm:pt-12 relative overflow-hidden text-slate-300" x-data="{ 
        ...
        <div class="max-w-[90rem] mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            <div class="mb-8 flex flex-wrap items-end justify-between gap-6 border-b border-white/10 pb-6">
                <div class="space-y-2">
                    <div class="flex items-center gap-2 text-xs font-bold text-slate-500 uppercase tracking-widest">
                        <a href="{{ route('admin.posts.index') }}" class="hover:text-cyan-400 transition-colors">Admin Panel</a>
                ...
                <a href="{{ route('admin.posts.index') }}" class="px-5 py-2.5 rounded-xl text-sm font-bold text-slate-400 hover:text-white bg-slate-900 border border-white/10 hover:border-white/20 hover:bg-slate-800 transition-all shadow-lg">
                    Back to Posts
                </a>
            </div>
            ...
            <form id="post-form" action="{{ route('admin.posts.update', $post->id) }}" method="POST" enctype="multipart/form-data" @submit.prevent="submitForm">
                @csrf
                @method('PATCH')
        ...
        <div class="fixed bottom-0 left-0 right-0 z-50 bg-slate-950/80 backdrop-blur-xl border-t border-white/10 py-4 shadow-[0_-10px_40px_rgba(0,0,0,0.5)]">
            <div class="max-w-[90rem] mx-auto px-4 sm:px-6 lg:px-8 flex flex-col sm:flex-row items-center justify-end gap-4">
                <div class="flex items-center w-full sm:w-auto gap-4">
                    <a href="{{ route('admin.posts.index') }}" class="flex-1 sm:flex-none text-center px-6 py-3 rounded-xl font-bold text-slate-400 hover:text-white bg-slate-900 border border-white/5 hover:border-white/20 transition-all text-sm">Cancel</a>
