        ...
        <div class="max-w-[90rem] mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
                <div class="bg-slate-900/60 border border-white/10 p-6 rounded-[2rem] flex flex-col backdrop-blur-md shadow-xl relative overflow-hidden group">
                    ...
                    <form action="{{ route('admin.fetch-tasks.store') }}" method="POST" class="flex flex-col sm:flex-row gap-4 relative z-10">
                        @csrf
                        <div class="flex-grow">
                ...
                <div class="bg-slate-900/60 border border-white/10 p-6 rounded-[2rem] flex flex-col backdrop-blur-md shadow-xl relative overflow-hidden group">
                    ...
                    <form action="{{ route('admin.fetch-tasks.populate') }}" method="POST" class="flex flex-col sm:flex-row items-start sm:items-center gap-4 mt-auto relative z-10">
                        @csrf
                            ...
                            <tr class="hover:bg-white/[0.02] transition-colors group">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right">
                                    <div class="flex items-center justify-end gap-2">
                                        <form action="{{ route('admin.fetch-tasks.reset', ['id' => $task->id]) }}" method="POST" onsubmit="return confirm('Reset task: {{ $task->tag_name }}?');" class="inline-block">
                                            @csrf
                                        ...
                                        <form action="{{ route('admin.fetch-tasks.destroy', ['id' => $task->id]) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete {{ $task->tag_name }}?');" class="inline-block">
                                            @csrf
                                            @method('DELETE')
