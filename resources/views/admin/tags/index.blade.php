...
<body class="bg-slate-950 text-slate-200 font-sans min-h-screen flex flex-col selection:bg-cyan-500 selection:text-white">
        ...
        <div class="max-w-[90rem] mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            ...
            <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-5 mb-12 relative z-10">
                @forelse($tags as $tag)
                    ...
                    <a href="{{ route('admin.tags.edit', ['id' => $tag->id]) }}" class="group block relative rounded-[1.5rem] overflow-hidden border border-white/5 bg-slate-900/60 shadow-lg hover:shadow-[0_0_20px_rgba(34,211,238,0.15)] hover:border-cyan-500/30 transition-all duration-300 backdrop-blur-sm transform hover:-translate-y-1">
    ...
    <script>
        ...
        window.toggleDebug = async function(tagId, isDebug) {
            try {
                const urlTemplate = "{{ route('admin.tags.toggle-debug', ['id' => '__ID__']) }}";
        ...
        window.toggleRatingDropdown = async function(tagId, event) {
            ...
            if (selectedRating && selectedRating !== currentRating) {
                try {
                    ...
                    const urlTemplate = "{{ route('admin.tags.update-rating', ['id' => '__ID__']) }}";
                    const url = urlTemplate.replace('__ID__', tagId);
                    const response = await fetch(url, {
                        method: 'PATCH',
                        headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': csrfToken, 'Accept': 'application/json', 'X-Requested-With': 'XMLHttpRequest' },
