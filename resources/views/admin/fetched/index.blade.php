...
<body class="bg-slate-950 text-slate-200 font-sans min-h-screen flex flex-col selection:bg-cyan-500 selection:text-white">
    ...
    <main class="flex-grow pt-8 pb-32 sm:pt-12 relative overflow-hidden text-slate-300" x-data="moderationPanel()">
        ...
        <section id="admin-management" class="max-w-[90rem] mx-auto px-4 sm:px-6 lg:px-8 py-12 relative z-10">
            ...
            <form method="GET" action="{{ route('admin.fetch-wallpapers.index') }}" class="relative mb-8 z-30 group">
                ...
                <div class="relative z-10 flex flex-col lg:flex-row gap-5 items-end p-5 sm:p-6">
                    ...
                    <div class="w-full lg:w-auto flex gap-3 relative z-10">
                        ...
                        @if(request()->anyFilled(['search', 'rating', 'source']) || $currentStatus != 'pending')
                            <a href="{{ route('admin.fetch-wallpapers.index') }}" class="flex-1 lg:flex-none px-5 py-3 bg-slate-800 hover:bg-red-500/20 border border-transparent hover:border-red-500/30 text-slate-300 hover:text-red-400 font-bold rounded-xl transition-all text-center flex items-center justify-center outline-none">
                                Clear
                            </a>
                        @endif
                    </div>
        ...
        document.addEventListener('alpine:init', () => {
            Alpine.data('moderationPanel', () => ({
                ...
                async checkApprove(id, itemData) {
                    ...
                    try {
                        const url = `{{ route('admin.fetch-wallpapers.approve', ':id') }}`.replace(':id', id);
                ...
                async forceApprove(event) {
                    ...
                    try {
                        const url = `{{ route('admin.fetch-wallpapers.approve', ':id') }}?force=true`.replace(':id', id);
                        const response = await fetch(url, { method: 'POST', headers: { 'X-CSRF-TOKEN': this.csrfToken, 'Content-Type': 'application/json', 'Accept': 'application/json' } });
                        const data = await response.json();
                ...
                async replaceItem(targetId, event) {
                    ...
                    try {
                        let url = `{{ route('admin.fetch-wallpapers.replace', ['id' => ':fetchId', 'target_id' => ':targetId']) }}`.replace(':fetchId', fetchId).replace(':targetId', targetId);
                        const response = await fetch(url, { method: 'POST', headers: { 'X-CSRF-TOKEN': this.csrfToken, 'Content-Type': 'application/json', 'Accept': 'application/json' } });
                        const data = await response.json();
                ...
                async rejectItem(id) {
                    try {
                        const url = `{{ route('admin.fetch-wallpapers.reject', ':id') }}`.replace(':id', id);
                        const response = await fetch(url, { method: 'POST', headers: { 'X-CSRF-TOKEN': this.csrfToken, 'Content-Type': 'application/json', 'Accept': 'application/json' } });
                ...
                async deleteItem(id) {
                    ...
                    try {
                        const url = `{{ route('admin.fetch-wallpapers.destroy', ':id') }}`.replace(':id', id);
