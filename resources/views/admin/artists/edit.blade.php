    ...
    <main class="flex-grow pt-8 pb-32 sm:pt-12 relative overflow-hidden text-slate-300" x-data="{ 
        ...
        async submitForm(e) { 
            ...
            try { 
                ...
                if (response.ok) { 
                    ...
                    if(resData.slug) { 
                        const newUrl = `{{ route('admin.artists.edit', ':id') }}`.replace(':id', {{ $artist->id }}); 
        ...
        <div class="max-w-[90rem] mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            <div class="mb-10 flex flex-wrap items-end justify-between gap-6 border-b border-white/10 pb-6">
                <div class="space-y-2">
                    <div class="flex items-center gap-2 text-xs font-bold text-slate-500 uppercase tracking-widest">
                        <a href="{{ route('admin.artists.index') }}" class="hover:text-cyan-400 transition-colors">Artists</a>
                ...
                <a href="{{ route('admin.artists.index') }}" class="px-5 py-2.5 rounded-xl text-sm font-bold text-slate-400 hover:text-white bg-slate-900 border border-white/10 hover:border-white/20 hover:bg-slate-800 transition-all shadow-lg">
                    Back to List
                </a>
            </div>
            ...
            <div class="flex flex-col lg:grid lg:grid-cols-12 gap-8 lg:gap-10">
                ...
                <div class="lg:col-span-8 space-y-8">
                    <form id="artistForm" action="{{ route('admin.artists.update', $artist->id) }}" method="POST" enctype="multipart/form-data" @submit.prevent="submitForm" class="space-y-8">
                        @csrf
                        @method('PATCH')
                        ...
                        <div x-data="{ 
                            ...
                            async generateAndCopyPrompt() { 
                                ...
                                try { 
                                    const response = await fetch('{{ route('admin.seo.artists', $artist->id) }}', { headers: { 'Accept': 'application/json', 'X-Requested-With': 'XMLHttpRequest' } }); 
        ...
        <div class="fixed bottom-0 left-0 right-0 z-50 bg-slate-950/80 backdrop-blur-xl border-t border-white/10 py-4 shadow-[0_-10px_40px_rgba(0,0,0,0.5)]">
            <div class="max-w-[90rem] mx-auto px-4 sm:px-6 lg:px-8 flex flex-col sm:flex-row items-center justify-between gap-4">
                <form action="{{ route('admin.artists.delete', $artist->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to permanently delete this artist? This action cannot be undone.');" class="w-full sm:w-auto">
                    @csrf @method('DELETE')
                ...
                <div class="flex items-center w-full sm:w-auto gap-4">
                    <a href="{{ route('admin.artists.index') }}" class="flex-1 sm:flex-none text-center px-6 py-3 rounded-xl font-bold text-slate-400 hover:text-white bg-slate-900 border border-white/5 hover:border-white/20 transition-all text-sm">Cancel</a>
