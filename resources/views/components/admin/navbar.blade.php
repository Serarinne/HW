@php
    $navLinks = [
        ['name' => 'Dashboard', 'route' => 'admin.index', 'pattern' => 'admin.index'],
        ['name' => 'Wallpapers', 'route' => 'admin.wallpapers.index', 'pattern' => 'admin.wallpapers.*'],
        ['name' => 'Characters', 'route' => 'admin.characters.index', 'pattern' => 'admin.characters.*'],
        ['name' => 'Series', 'route' => 'admin.series.index', 'pattern' => 'admin.series.*'],
        ['name' => 'Artists', 'route' => 'admin.artists.index', 'pattern' => 'admin.artists.*'],
        ['name' => 'Tags', 'route' => 'admin.tags.index', 'pattern' => 'admin.tags.*'],
        ['name' => 'Blog', 'route' => 'admin.posts.index', 'pattern' => 'admin.posts.*'],
        ['name' => 'Redeems', 'route' => 'admin.redeems.index', 'pattern' => 'admin.redeems.*'],
        ['name' => 'Fetch Tasks', 'route' => 'admin.fetch-tasks.index', 'pattern' => 'admin.fetch-tasks.*'],
        ['name' => 'Fetch WP', 'route' => 'admin.fetch-wallpapers.index', 'pattern' => 'admin.fetch-wallpapers.*'],
    ];
@endphp
...
<nav class="sticky top-0 z-50 bg-slate-950/95 backdrop-blur border-b border-white/10" x-data="{ open: false }" aria-label="Admin Navigation">
    <div class="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between gap-4">
        ...
        <a href="{{ route('admin.index') }}" class="flex-shrink-0 flex items-center" aria-label="{{ env('APP_NAME') }} Admin">
        ...
        <div class="hidden lg:flex items-center gap-2 flex-shrink-0">
            ...
            <a href="{{ route('admin.settings') }}"
               class="flex items-center gap-1.5 text-sm font-medium px-3 py-2 rounded-lg transition-colors {{ request()->routeIs('admin.settings') ? 'text-white bg-white/10' : 'text-slate-300 hover:text-white hover:bg-white/5' }}"
            ...
            <a href="{{ route('home') }}" target="_blank"
               class="flex items-center gap-1.5 text-sm font-medium px-3 py-2 rounded-lg transition-colors text-slate-300 hover:text-white hover:bg-white/5"
    ...
    <div id="mobile-menu"
        ...
        <div class="max-w-[1400px] mx-auto px-4 py-4 flex flex-col gap-1 text-sm font-medium">
            ...
            <a href="{{ route('admin.settings') }}"
               class="{{ request()->routeIs('admin.settings') ? 'text-white bg-cyan-400/20 border-l-2 border-cyan-400' : 'text-slate-400 hover:text-white hover:bg-white/5' }} px-3 py-2.5 rounded-lg transition-colors flex items-center gap-2">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z"></path>
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path>
                </svg>
            ...
            {{-- Back to Site (Mobile) --}}
            <a href="{{ route('home') }}"
               class="text-slate-400 hover:text-white hover:bg-white/5 px-3 py-2.5 rounded-lg transition-colors flex items-center gap-2">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"></path>
                </svg>
                Back to Site
