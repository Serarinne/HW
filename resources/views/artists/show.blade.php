...
<html lang="en-US" class="scroll-smooth bg-slate-950">
  <head>
    ...
    <title>{{ $artist->seo_title ?? $artist->name . ' HSR Art & Illustrations' }} - {{ env('APP_NAME') }}</title>
    <meta name="description" content="{{ $artist->seo_description ?? 'Dive into a stunning collection of ' . $artist->name . ' Honkai: Star Rail wallpapers and illustrations. Personalize your device with HD and 4K HSR digital art from this creator.' }}" />
    
    @if(!empty($artist->seo_keywords))
    ...
    <meta property="og:title" content="{{ $artist->name }} HSR Art & Illustrations | {{ env('APP_NAME') }}" />
    <meta property="og:description" content="{{ $artist->seo_description ?? 'Explore our stunning collection of ' . $artist->name . ' Honkai: Star Rail illustrations and fanart in HD and 4K resolution.' }}" />
    <meta property="og:url" content="{{ url()->current() }}" />
    <meta property="og:type" content="profile" />
    <meta property="og:profile:username" content="{{ $artist->slug }}" />
    <meta property="og:site_name" content="{{ env('APP_NAME') }}" />
    @if(!empty($artist->image))
    <meta property="og:image" content="{{ $artist->image['jpg'] ?? $artist->image['webp'] }}" />
    @endif
    ...
    [
      ...
      },
      {
        ...
        "mainEntity": {
          "@@type": "Person",
          "name": "{{ $artist->name }}",
          "description": "Honkai: Star Rail fanart creator and illustrator.",
          @if(!empty($artist->image))
    ...
    <main class="flex-grow relative w-full text-slate-300 pt-8 pb-32">
        <div class="max-w-[90rem] mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            ...
            <div class="mb-8 flex items-center gap-2 text-xs font-bold text-slate-300 uppercase tracking-widest backdrop-blur-md bg-slate-900/80 w-fit px-5 py-2.5 rounded-full border border-white/10 shadow-sm">
                ...
                <a href="{{ route('artists.index') }}" class="hover:text-cyan-400 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-400 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-900 rounded-sm">Creators</a>
                <span class="text-slate-500">&bull;</span>
                <span class="text-cyan-400 truncate max-w-[200px] sm:max-w-md">{{ $artist->name }}</span>
            </div>
            ...
            <section class="mb-12 sm:mb-16" x-data="{ descriptionOpen: false }">
                <div class="flex flex-col md:flex-row gap-8 lg:gap-12 items-center md:items-start bg-slate-900/50 backdrop-blur-md border border-white/10 p-6 sm:p-10 rounded-3xl shadow-2xl relative overflow-hidden group">
                        ...
                        <div class="relative w-40 h-40 sm:w-48 sm:h-48 md:w-56 md:h-56 lg:w-64 lg:h-64 rounded-full overflow-hidden bg-slate-950 border-4 border-slate-900 shadow-[0_0_30px_rgba(168,85,247,0.2)] group-hover:shadow-[0_0_40px_rgba(34,211,238,0.3)] transition-all duration-500 ring-2 ring-white/10 group-hover:ring-cyan-500/50 {{ !$hasImage ? 'flex justify-center items-center' : '' }}">
                            @if($hasImage)
                                <picture class="absolute inset-0 w-full h-full block group-hover:scale-110 transition-transform duration-700 ease-out">
                                    ...
                                    <img src="{{ $artist->image['jpg'] }}" alt="{{ $artist->name }} HSR Art Profile" loading="eager" width="400" height="400" class="absolute inset-0 w-full h-full object-cover bg-slate-800" />
                                </picture>
                    ...
                    <div class="flex-grow w-full text-center md:text-left relative z-10 flex flex-col justify-center">
                        <h1 class="text-4xl sm:text-5xl lg:text-6xl font-black text-white mb-6 tracking-tight drop-shadow-md">
                            {{ $artist->name }}
                        </h1>
                        ...
                        @if(!empty($artist->description))
                            <div class="text-slate-400 text-sm lg:text-base max-w-3xl relative font-medium leading-relaxed bg-slate-950/50 p-5 rounded-2xl border border-white/5 shadow-inner">
                                <div id="artist-description" :class="descriptionOpen ? '' : 'line-clamp-3 lg:line-clamp-4'" class="transition-all duration-300">
            ...
            <section>
                <div class="flex flex-col sm:flex-row items-start sm:items-end justify-between mb-8 gap-4 border-b border-white/10 pb-4">
                    <div class="space-y-2">
                        <p class="text-xs font-bold uppercase tracking-widest text-cyan-400 flex items-center gap-2">
                            ...
                            HSR Illustrations by {{ $artist->name }}
                        </p>
                        <h2 class="text-2xl sm:text-3xl font-black text-white tracking-tight">
                            Creator's <span class="text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-cyan-400">Gallery</span>
                        </h2>
                ...
                @else
                    <div class="flex flex-col items-center justify-center py-20 px-4 text-center bg-slate-900/50 border border-dashed border-slate-700/50 rounded-3xl backdrop-blur-sm w-full max-w-3xl mx-auto">
                        ...
                        <h3 class="text-2xl font-bold text-white mb-3">No Records in Data Bank</h3>
                        <p class="text-base text-slate-400 max-w-md mx-auto leading-relaxed">No HSR wallpapers have been uploaded from this creator yet.</p>
                    </div>
                @endif
