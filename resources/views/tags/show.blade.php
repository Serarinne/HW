...
<html lang="en-US" class="scroll-smooth bg-slate-950">
  <head>
    ...
    <title>{{ $tag->seo_title ?? $tag->name . ' HSR Wallpapers HD & 4K' }} - {{ env('APP_NAME') }}</title>
    <meta name="description" content="{{ $tag->seo_description ?? 'Download the best ' . $tag->name . ' wallpapers. Explore a stunning collection of HD and 4K Honkai: Star Rail character and faction backgrounds for desktop and mobile.' }}" />
    ...
    <meta property="og:title" content="{{ $tag->name }} HSR Wallpapers | {{ env('APP_NAME') }}" />
    <meta property="og:description" content="{{ $tag->seo_description ?? 'Explore our stunning collection of Honkai: Star Rail ' . $tag->name . ' wallpapers in HD and 4K resolution.' }}" />
    <meta property="og:url" content="{{ url()->current() }}" />
    <meta property="og:type" content="website" />
    <meta property="og:site_name" content="{{ env('APP_NAME') }}" />
    @if(!empty($tag->image))
    <meta property="og:image" content="{{ $tag->image['jpg'] ?? $tag->image['webp'] }}" />
    @endif
    ...
    <main class="flex-grow relative w-full text-slate-300 pt-8 pb-32">
        <div class="max-w-[90rem] mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            ...
            <div class="mb-8 flex flex-wrap items-center gap-2 text-xs font-bold text-slate-400 uppercase tracking-widest backdrop-blur-md bg-slate-900/80 w-fit px-5 py-2.5 rounded-full border border-white/10 shadow-sm">
                ...
                <a href="{{ route('tags.index') }}" class="hover:text-cyan-400 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-400 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-900 rounded-sm">Tags</a>
                <span class="text-slate-600">&bull;</span>
                <span class="text-cyan-400 truncate max-w-[120px] sm:max-w-[200px]">{{ $tag->name }}</span>
            </div>
            ...
            <section class="mb-12 sm:mb-16" x-data="{ descriptionOpen: false }">
                <div class="flex flex-col md:flex-row gap-8 lg:gap-12 items-center md:items-start bg-slate-900/50 backdrop-blur-md border border-white/10 p-6 sm:p-10 rounded-3xl shadow-2xl relative overflow-hidden group">
                    ...
                    <div class="absolute -right-24 -top-24 w-64 h-64 bg-cyan-500/10 rounded-full blur-[80px] pointer-events-none group-hover:bg-cyan-500/10 transition-all duration-700"></div>
                    ...
                    <div class="flex-shrink-0 relative z-10">
                        <div class="relative w-40 h-40 sm:w-48 sm:h-48 md:w-56 md:h-56 lg:w-64 lg:h-64 rounded-full overflow-hidden bg-slate-950 border-4 border-slate-900 shadow-[0_0_30px_rgba(34,211,238,0.2)] group-hover:shadow-[0_0_40px_rgba(34,211,238,0.4)] transition-all duration-500 ring-2 ring-white/10 group-hover:ring-cyan-500/50 {{ !$hasImage ? 'flex justify-center items-center' : '' }}">
                            @if($hasImage)
                                <picture class="absolute inset-0 w-full h-full block group-hover:scale-110 transition-transform duration-700 ease-out">
                                    ...
                                    <img src="{{ $tag->image['jpg'] }}" alt="{{ $tag->name }} HSR Wallpapers" loading="eager" width="400" height="400" class="absolute inset-0 w-full h-full object-cover bg-slate-800" />
                                </picture>
                            ...
                            @else
                                <div class="z-10 w-full h-full bg-gradient-to-br from-slate-800 to-slate-900 rounded-full flex items-center justify-center text-cyan-400 font-black text-6xl uppercase shadow-lg border border-white/5">
                                    {{ $firstLetter }}
                                </div>
                            @endif
                        </div>
                    ...
                    <div class="flex-grow w-full text-center md:text-left relative z-10 flex flex-col justify-center">
                        <h1 class="text-4xl sm:text-5xl lg:text-6xl font-black text-white mb-6 tracking-tight drop-shadow-md flex items-center justify-center md:justify-start gap-2">
                            <span class="text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-cyan-500 opacity-80 font-light">#</span>{{ $tag->name }}
                        </h1>
                        ...
                        @if(!empty($tag->description))
                            <div class="text-slate-400 text-sm lg:text-base max-w-3xl relative font-medium leading-relaxed bg-slate-950/50 p-5 rounded-2xl border border-white/5 shadow-inner">
                                <div id="tag-description" :class="descriptionOpen ? '' : 'line-clamp-3 lg:line-clamp-4'" class="transition-all duration-300">
            ...
            <div class="w-full mb-12 empty:hidden flex justify-center items-center min-h-[90px]">
                <x-ads />
            </div>
            ...
            <section>
                <div class="flex flex-col sm:flex-row items-start sm:items-end justify-between mb-8 gap-4 border-b border-white/10 pb-4">
                    <div class="space-y-2">
                        <p class="text-xs font-bold uppercase tracking-widest text-cyan-400 flex items-center gap-2">
                            <span class="w-2.5 h-2.5 rounded-full bg-cyan-400 animate-pulse shadow-[0_0_8px_rgba(34,211,238,0.8)]"></span> 
                            HSR Data Bank Tags
                        </p>
                        <h2 class="text-2xl sm:text-3xl font-black text-white tracking-tight">
                            <span class="text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-cyan-500">{{ $tag->name }}</span> Wallpapers
                        </h2>
                ...
                @else
                    <div class="flex flex-col items-center justify-center py-20 px-4 text-center bg-slate-900/50 border border-dashed border-slate-700/50 rounded-3xl backdrop-blur-sm w-full max-w-3xl mx-auto">
                        ...
                        <p class="text-base text-slate-400 max-w-md mx-auto leading-relaxed">No HSR wallpapers available for this tag yet.</p>
                    </div>
                @endif
